...
export default function OrgLogin(){
  ...
  const onSubmit=async e=>{
    ...
    setLoading(true)
    try{const res=await authAPI.orgLogin(form);loginOrg(res.data);navigate('/organizer/dashboard')}
    catch(err){setError(err.response?.data?.message||t('err_login_failed',lang))}
    finally{setLoading(false)}
  }
  return(
    <AuthCard title={t('h_org_login','en')} titleKn={t('h_org_login','kn')} subtitle={t('org_login_subtitle',lang)} error={error} bottomText={t('not_registered_yet',lang)} bottomLinkTo="/organizer/register" bottomLinkLabel={t('register_here_btn',lang)}>
